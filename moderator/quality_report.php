<?php
session_start();

include("../config.php");

if(!isset($_SESSION['user_id']))
{
    header("Location: ../login.php");
    exit();
}

if($_SESSION['role'] != 'moderator')
{
    header("Location: ../login.php");
    exit();
}


$recipes_query = mysqli_query($conn,
"SELECT * FROM recipes");

$total_recipes = mysqli_num_rows($recipes_query);


$published = 0;
$draft = 0;

while($recipe = mysqli_fetch_assoc($recipes_query))
{
    if($recipe['status'] == 'published')
    {
        $published++;
    }

    if($recipe['status'] == 'draft')
    {
        $draft++;
    }
}


$reports_query = mysqli_query($conn,
"SELECT * FROM content_reports");

$total_reports = mysqli_num_rows($reports_query);

$cuisines_query = mysqli_query($conn,
"SELECT * FROM cuisines");

$chef_query = mysqli_query($conn,
"SELECT * FROM chef_verification_requests");

$approved = 0;
$rejected = 0;
$pending = 0;

while($chef = mysqli_fetch_assoc($chef_query))
{
    if($chef['status'] == 'approved')
    {
        $approved++;
    }

    if($chef['status'] == 'rejected')
    {
        $rejected++;
    }

    if($chef['status'] == 'pending')
    {
        $pending++;
    }
}

?>

<!DOCTYPE html>
<html>

<head>

<title>Quality Report</title>

<style>

body{
    margin:0;
    font-family:Arial;
    background:#FFF8F2;
}

.sidebar{
    width:230px;
    height:100vh;
    background:#FFEADD;
    position:fixed;
    left:0;
    top:0;
    padding-top:20px;
}

.sidebar h2{
    text-align:center;
    color:#5A4636;
}

.sidebar a{
    display:block;
    padding:12px 20px;
    text-decoration:none;
    color:#5A4636;
    margin:8px;
    border-radius:15px;
}

.sidebar a:hover{
    background:#FFD6C9;
}

.active{
    background:#FFD6C9;
}

.main{
    margin-left:250px;
    padding:20px;
}

.card{
    background:white;
    padding:20px;
    border-radius:20px;
    margin-bottom:20px;
    box-shadow:0px 2px 8px rgba(0,0,0,0.1);
}

h1,h2{
    color:#5A4636;
}

</style>

</head>

<body>

<div class="sidebar">

<h2>Moderator</h2>

<a href="dashboard.php">Dashboard</a>
<a href="verification.php">Chef Verification</a>
<a href="recipes.php">Recipes</a>
<a href="reports.php">Reports</a>
<a href="review_report.php">Review Reports</a>
<a href="cuisines.php">Cuisines</a>
<a href="diet_types.php">Diet Types</a>
<a href="profile.php">Profile</a>
<a href="quality_report.php" class="active">Quality Report</a>
<a href="moderation_logs.php">Moderation Logs</a>
<a href="../logout.php">Logout</a>

</div>

<div class="main">

<h1>Content Quality Report</h1>


<div class="card">

<h2>Total Recipes</h2>

<p><?php echo $total_recipes; ?></p>

</div>


<div class="card">

<h2>Recipe Status Breakdown</h2>

<p>Published : <?php echo $published; ?></p>

<p>Draft : <?php echo $draft; ?></p>

</div>


<div class="card">

<h2>Total Reports</h2>

<p><?php echo $total_reports; ?></p>

</div>


<div class="card">

<h2>Most Reported Cuisines</h2>

<?php

while($cuisine = mysqli_fetch_assoc($cuisines_query))
{
    $cuisine_id = $cuisine['id'];

    $count = 0;

    $recipe_query = mysqli_query($conn,
    "SELECT * FROM recipes
    WHERE cuisine_id='$cuisine_id'");

    while($recipe = mysqli_fetch_assoc($recipe_query))
    {
        $recipe_id = $recipe['id'];

        $report_query = mysqli_query($conn,
        "SELECT * FROM content_reports
        WHERE entity_id='$recipe_id'
        AND entity_type='recipe'");

        $count += mysqli_num_rows($report_query);
    }

?>

<p>

<?php echo $cuisine['name']; ?> :

<?php echo $count; ?>

</p>

<?php } ?>

</div>


<div class="card">

<h2>Chef Verification Status</h2>

<p>Approved : <?php echo $approved; ?></p>

<p>Rejected : <?php echo $rejected; ?></p>

<p>Pending : <?php echo $pending; ?></p>

</div>

</div>

</body>
</html>